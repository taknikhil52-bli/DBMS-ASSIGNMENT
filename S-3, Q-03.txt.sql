-- Section 3 Q3 - Parameterised Cursor by Price Range
-- Roll No - 12
-- Name - Kirtan Gohel
...
DECLARE
    CURSOR c_price_range (p_min NUMBER, p_max NUMBER) IS
        SELECT title, price FROM book WHERE price BETWEEN p_min AND p_max;

    v_min NUMBER := 300;
    v_max NUMBER := 700;
BEGIN
    DBMS_OUTPUT.PUT_LINE('Books priced between Rs.' || v_min || ' and Rs.' || v_max || ':');
    FOR r IN c_price_range(v_min, v_max) LOOP
