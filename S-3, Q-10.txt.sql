-- Section 3 Q10 - Overdue Fine Calculator
-- Roll No - 06
...
DECLARE
    CURSOR c_overdue (p_days NUMBER) IS
        SELECT issue_id, book_id, issue_date,
               TRUNC(SYSDATE - issue_date) AS days_overdue
        FROM   book_issue
        WHERE  return_date IS NULL
        AND    TRUNC(SYSDATE - issue_date) > p_days;

    c_fine_per_day CONSTANT NUMBER := 2;
    v_fine      NUMBER;
    v_total_fine NUMBER := 0;
    v_days_limit NUMBER := 30;
BEGIN
    DBMS_OUTPUT.PUT_LINE('Issues overdue by more than ' || v_days_limit || ' days:');
    FOR r IN c_overdue(v_days_limit) LOOP
        v_fine := r.days_overdue * c_fine_per_day;
        v_total_fine := v_total_fine + v_fine;
        DBMS_OUTPUT.PUT_LINE('Issue ' || r.issue_id || ' | Book ' || r.book_id ||
                             ' | Overdue ' || r.days_overdue || ' days | Fine: Rs.' || v_fine);
    END LOOP;
