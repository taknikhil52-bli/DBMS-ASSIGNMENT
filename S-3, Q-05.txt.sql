-- Section 3 Q5 - Parameterised Cursor with DEFAULT Value
-- Roll No - 12
-- Name - Kirtan Gohel
...
DECLARE
    -- Parameterised cursor with default value 'Database'
    CURSOR c_cat (p_cat VARCHAR2 DEFAULT 'Database') IS
        SELECT title, price 
        FROM book 
        WHERE category = p_cat;
BEGIN
    -- 1. Default parameter value ('Database') test karna
    DBMS_OUTPUT.PUT_LINE('--- Books in Category: Database (DEFAULT) ---');
    FOR r IN c_cat LOOP
        DBMS_OUTPUT.PUT_LINE(r.title || ' - Rs.' || r.price);
    END LOOP;

    DBMS_OUTPUT.PUT_LINE(CHR(10) || '--- Books in Category: Networking ---');
    -- 2. Explicit parameter value ('Networking') pass karna
    FOR r IN c_cat('Networking') LOOP
        DBMS_OUTPUT.PUT_LINE(r.title || ' - Rs.' || r.price);
    END LOOP;
END;
/