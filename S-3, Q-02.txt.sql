-- Section 3 Q2 - Parameterised Cursor by Publisher Name
-- Roll No - 12
-- Name - Kirtan Gohel
...
DECLARE
    CURSOR c_by_pub (p_pub_name VARCHAR2) IS
        SELECT b.title, b.price
        FROM   book b
        JOIN   publisher p ON b.pub_id = p.pub_id
        WHERE  UPPER(p.pub_name) = UPPER(p_pub_name);

    v_pub_name VARCHAR2(40) := 'mcgraw hill';
BEGIN
    DBMS_OUTPUT.PUT_LINE('Books published by: ' || v_pub_name);
    FOR r IN c_by_pub(v_pub_name) LOOP
