-- Section 3 Q6 - Parameterised Cursor by Member ID (3-table join)
-- Roll No - 06
...
DECLARE
    CURSOR c_member_books (p_member_id NUMBER) IS
...
BEGIN
    DBMS_OUTPUT.PUT_LINE('Books issued to member ID ' || v_member_id || ':');
    FOR r IN c_member_books(v_member_id) LOOP
        DBMS_OUTPUT.PUT_LINE(r.title || ' | Issued: ' || TO_CHAR(r.issue_date,'DD-MON-YYYY'));
    END LOOP;
END;
/