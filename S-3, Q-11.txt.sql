-- Section 3 Q11 - FOR UPDATE Price Hike by Category
-- Roll No - 06
...
DECLARE
    CURSOR c_price_hike (p_cat VARCHAR2) IS
    ...
    v_old_price NUMBER;
    v_new_price NUMBER;
    v_category  VARCHAR2(25) := 'Programming';
BEGIN
    FOR r IN c_price_hike(v_category) LOOP
        v_old_price := r.price;
        v_new_price := ROUND(r.price * 1.10, 2);

        UPDATE book
        SET    price = v_new_price
        WHERE  CURRENT OF c_price_hike;

        DBMS_OUTPUT.PUT_LINE(r.title || ' | Old Price: Rs.' || v_old_price ||
