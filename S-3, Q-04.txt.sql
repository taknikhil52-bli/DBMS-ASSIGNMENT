-- Section 3 Q4 - Parameterised Cursor by Course and Semester
-- Roll No - 06
...
DECLARE
    CURSOR c_by_course_sem (p_course VARCHAR2, p_sem NUMBER) IS
        SELECT member_name, join_date
        FROM   lib_member
        WHERE  course = p_course AND semester = p_sem;

    v_course VARCHAR2(20) := 'MCA';
    v_sem    NUMBER := 2;
BEGIN
    DBMS_OUTPUT.PUT_LINE('Members of ' || v_course || ' Semester ' || v_sem || ':');
    FOR r IN c_by_course_sem(v_course, v_sem) LOOP
        DBMS_OUTPUT.PUT_LINE(r.member_name || ' | Joined: ' || TO_CHAR(r.join_date,'DD-MON-YYYY'));
    END LOOP;
END;
/