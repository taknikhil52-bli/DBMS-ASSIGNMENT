-- Section 3 Q1 - Parameterised Cursor by Category
-- Roll No - 06
...
ACCEPT p_category CHAR PROMPT 'Enter category (e.g. Database, Programming): ';

DECLARE
    CURSOR c_by_cat (p_cat VARCHAR2) IS
        SELECT book_id, title, price FROM book WHERE category = p_cat;

    v_category VARCHAR2(25) := '&p_category';
BEGIN
    DBMS_OUTPUT.PUT_LINE('Books in category: ' || v_category);
    FOR r IN c_by_cat(v_category) LOOP
