-- Section 3 Q7 - Nested Cursors (Publisher and its Books)
-- Roll No - 12
-- Name - Kirtan Gohel
...
DECLARE
    CURSOR c_pub IS
        SELECT pub_id, pub_name FROM publisher;

    CURSOR c_books_of_pub (p_pub_id NUMBER) IS
        SELECT title, price FROM book WHERE pub_id = p_pub_id;
BEGIN
    FOR p IN c_pub LOOP
        DBMS_OUTPUT.PUT_LINE(p.pub_name || ':');
        FOR b IN c_books_of_pub(p.pub_id) LOOP
