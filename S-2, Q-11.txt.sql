-- Section 2 Q11 - Top 5 Priced Books using %ROWCOUNT
-- Roll No - 06
...
BEGIN
    OPEN c_top_price;
    LOOP
        FETCH c_top_price INTO v_title, v_price;
        EXIT WHEN c_top_price%NOTFOUND;

        DBMS_OUTPUT.PUT_LINE(c_top_price%ROWCOUNT || '. ' || v_title || ' - Rs.' || v_price);

        EXIT WHEN c_top_price%ROWCOUNT = 5;
    END LOOP;
    CLOSE c_top_price;
END;
/