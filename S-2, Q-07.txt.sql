-- Section 2 Q7 - %ISOPEN Cursor Check
-- Roll No - 06
...
BEGIN
    ...
    ELSE
        DBMS_OUTPUT.PUT_LINE('Cursor not open yet - opening now.');
        OPEN c_book;
    END IF;

    DBMS_OUTPUT.PUT_LINE('Is cursor open now? ' || CASE WHEN c_book%ISOPEN THEN 'TRUE' ELSE 'FALSE' END);

    CLOSE c_book;

    DBMS_OUTPUT.PUT_LINE('Is cursor open after CLOSE? ' || CASE WHEN c_book%ISOPEN THEN 'TRUE' ELSE 'FALSE' END);
END;
/