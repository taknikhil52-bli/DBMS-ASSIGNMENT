-- Section 3 Q8 - Parameterised Cursor by Country with BOOLEAN Flag
-- Roll No - 06
...
DECLARE
    CURSOR c_by_country (p_country VARCHAR2) IS
        SELECT pub_name, city FROM publisher WHERE country = p_country;

    v_country VARCHAR2(30) := 'Australia';
    v_found   BOOLEAN := FALSE;
BEGIN
    FOR r IN c_by_country(v_country) LOOP
        v_found := TRUE;
        DBMS_OUTPUT.PUT_LINE(r.pub_name || ' - ' || r.city);
    END LOOP;

    IF NOT v_found THEN
        DBMS_OUTPUT.PUT_LINE('No publisher found in ' || v_country);
    END IF;
