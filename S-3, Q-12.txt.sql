-- Section 3 Q12 - Parameterised Cursor by Starting Letter
-- Roll No - 06
...
DECLARE
    -- Letter yahan badal kar test karein ('R', 'A', 'N', etc.)
    v_letter VARCHAR2(1) := 'A'; -- Search Members Starting with ('R', 'A', 'N', etc.)

    CURSOR c_by_letter (p_char VARCHAR2) IS
        SELECT member_name, course
        FROM   lib_member
        WHERE  UPPER(SUBSTR(member_name, 1, 1)) = UPPER(p_char);

    v_found BOOLEAN := FALSE;
BEGIN
    DBMS_OUTPUT.PUT_LINE('Members starting with ' || UPPER(v_letter) || ':');

    FOR r IN c_by_letter(v_letter) LOOP
        v_found := TRUE;
        DBMS_OUTPUT.PUT_LINE('- ' || r.member_name || ' (' || r.course || ')');
    END LOOP;
