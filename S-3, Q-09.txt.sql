-- Section 3 Q9 - Parameterised Cursor by Month (EXTRACT)
-- Roll No - 06
...
DECLARE
    CURSOR c_by_month (p_month NUMBER) IS
    ...
    v_month NUMBER := 7;
BEGIN
    DBMS_OUTPUT.PUT_LINE('Books issued in month ' || v_month || ' of 2026:');
    FOR r IN c_by_month(v_month) LOOP
        DBMS_OUTPUT.PUT_LINE('Issue ' || r.issue_id || ' - ' || r.title ||
                             ' on ' || TO_CHAR(r.issue_date,'DD-MON-YYYY'));
    END LOOP;
END;
/